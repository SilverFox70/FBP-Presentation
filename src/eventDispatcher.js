...
var eventDispatcher = function(){
  ...
  return {
    ...
    emit : function(eventName, context, data){
      let prs = [...arguments];
      data = prs.slice(2);
      // console.log(`[eventDispatcher] Args: ${JSON.stringify(prs, null, 2)}, is an array: ${prs instanceof Array}`);
      ...
      // trying to figure out how parse the remaining args/data into proper args for second call
      // data = (data instanceof Array) ? data : [data];

      for (i = 0; i < subscribers.length; i++){
        if (subscribers[i].doOnce){
          subscribedFunctionsList.push(subscribers[i].fn); // for logging use
          subscribers[i].fn.apply(thisContext, data);
          this.off(eventName, subscribers[i]);
        } else {
          subscribedFunctionsList.push(subscribers[i]);    // for logging use
          console.log(`data in emit: ${JSON.stringify(data, null, 2)}, is an array ${data instanceof Array}`);
          subscribers[i].apply(thisContext, data);
        }
      }
      // console.log("!--------- logger is on: " + loggerOn);
      if (loggerOn) eventLogger.emitEvent(eventName, data, thisContext, subscribedFunctionsList);
    },
